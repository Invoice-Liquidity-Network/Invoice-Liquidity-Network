...
export async function flushPendingNotifications(): Promise<void> {
  ...
  for (const attempt of pending) {
    try {
      ...
      logSentNotification(
        attempt.payload.invoice.id,
        attempt.payload.trigger,
        attempt.payload.recipientAddress,
        attempt.subscription.channel,
        attempt.subscription.destination,
        attempt.payload.eventId
      );
    } catch (error) {
