...
`;
  ...
  function renderCommand(cmd: Command, parentNames: string[]) {
    ...
    // Usage
    const args = (cmd as any)._args ? (cmd as any)._args.map((a: any) => `${a.required ? '<' : '['}${a.name()}${a.required ? '>' : ']'}`).join(" ") : "";
    ...
    const afterHelp = customHelp.filter((h: any) => h.position === 'after').map((h: any) => typeof h.text === 'function' ? h.text() : h.text).join('\n');
