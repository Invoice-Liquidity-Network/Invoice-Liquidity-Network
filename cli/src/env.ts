...
export function registerEnvCommands(program: Command): void {
  const envCommand = program.command('env').description('Manage ILN environments');
  ...
  envCommand
    ...
    .action((options: { json?: boolean }) => {
      ...
      if (Object.keys(config.environments).length === 0) {
        if (options.json) {
          console.log(
            JSON.stringify({ success: true, data: { environments: {}, current: null } }, null, 2)
          );
        } else {
  ...
  envCommand
    ...
    .action((name: string) => {
      ...
      rl.question(
        `Are you sure you want to delete environment '${name}'? (yes/no): `,
        (answer: string) => {
          rl.close();
  ...
  envCommand
    ...
    .action((name: string, options: { json?: boolean }) => {
      ...
      if (!config.environments[name]) {
        if (options.json) {
          console.error(
            JSON.stringify({ success: false, error: `Environment '${name}' not found.` }, null, 2)
          );
        } else {
          console.error(`Error: Environment '${name}' not found.`);
        }
