...
| `InvalidAmountError` | `INVALID_AMOUNT` | Malformed or out-of-range amount/value input |
...
| `TransactionBuildError` | `TRANSACTION_BUILD_ERROR` | Failed to construct a valid transaction |
...
| `SimulationPreparedXdrMismatchError` | `SIMULATION_PREPARED_XDR_MISMATCH` | Prepared XDR differs from the simulated transaction |
| `GenericContractError` | `CONTRACT_ERROR` | Unclassified contract error |
| `InvalidContractResponseError` | `INVALID_CONTRACT_RESPONSE` | Contract returned an unparseable/unexpected payload |
| `OfflineQueueFullError` | `OFFLINE_QUEUE_FULL` | Offline queue reached its maximum size |
| `OfflineQueuedError` | `OFFLINE_QUEUED` | Offline operation could not be processed inline and was queued |
| `PluginError` | `PLUGIN_ERROR` | Plugin registration/invocation failed |
| `NotificationsApiError` | `NOTIFICATIONS_API_ERROR` | Notifications API request failed |
| `FederationResolutionError` | `FEDERATION_RESOLUTION_FAILED` | Federation address could not be resolved |
...
## API Snapshot & Breaking-Change Gate — Issue #1029

`sdk/api-snapshot.json` is a machine-readable snapshot of the public `@iln/sdk`
surface (every export reachable from `sdk/src/index.ts`, with member shapes,
signatures and enum values) built deterministically via
`scripts/generate-sdk-api-snapshot.mjs` using the TypeScript compiler API.

Workflow:

```bash
pnpm sdk:api:snapshot   # regenerates sdk/api-snapshot.json
pnpm sdk:api:check      # diffs current source against the committed snapshot
pnpm test:sdk-api       # unit-tests the classifier
```

`pnpm sdk:api:check`:

- exits 0 when the public surface is unchanged;
- exits 1 with a categorised report when it has changed. Breaking changes
  (removed exports, kind changes, enum member removal/revaluation, signature/
  type changes, newly-required params) additionally require a `major` changeset
  for `@iln/sdk` in `.changeset/`;
- treats additive changes (new exports/params/members) and union **narrowing**
  (e.g. `A | B` → `A`) as non-breaking.

CI: the `sdk-api-compat` job runs the check on SDK changes and comments on the
PR when drift or missing-`major`-changesets are detected. Whenever a release is
cut, regenerate the snapshot so the committed baseline reflects the released
surface.
