...
export function parseCsvHeader(line: string): string[] {
  return line.split(',').map((col) => col.trim().toLowerCase().replace(/['"]/g, ''));
}
...
export function parseCsv(content: string): { rows: CsvInvoiceRow[]; errors: CsvValidationError[] } {
  const errors: CsvValidationError[] = [];
  const lines = content
    .split('\n')
    .map((l) => l.trim())
...
export function formatBatchErrorSummary(
  ...
  for (const [reason, indices] of byReason) {
    summaries.push(
      `${indices.length} invoice(s) (rows ${indices
        .map((i) => i + 1)
        .join(', ')}) failed: ${reason}`
    );
  }
