  ...
  return (
    ...
    >
      ...
      <p style={{ margin: '0 0 20px 0', color: MUTED, fontSize: 13, lineHeight: 1.5 }}>
        File a claim against a defaulted invoice that you funded as an LP. The pool admin will
        review your claim and approve or reject it.
      </p>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {!preselectedId && (
          <div>
            ...
            {invLoading ? (
              ...
              >
                No defaulted invoices found. Claims can only be filed on invoices in Defaulted
                status.
              </div>
            ) : (
              <select
              ...
              >
                ...
                {defaultedInvoices.map((inv) => (
                  <option key={String(inv.id)} value={String(inv.id)}>
                    ...
                    — ${Number(inv.amount ?? 0n) / 10_000_000}
                  </option>
                ))}
              </select>
            )}
        ...
        {coverage && (
          ...
          >
            Your coverage:{' '}
            <strong style={{ color: TEXT }}>${Number(coverage.coverageAmount) / 10_000_000}</strong>
            &nbsp;| Premium rate:{' '}
            <strong style={{ color: TEXT }}>{(coverage.premiumRateBps / 100).toFixed(2)}%</strong>
