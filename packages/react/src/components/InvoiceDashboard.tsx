...
const ALL_METRICS: MetricKey[] = ['totalSubmitted', 'totalFunded', 'totalPaid', 'totalDefaulted'];
...
export function InvoiceDashboard({
  ...
  const applyEvent = useCallback(
    (event: LiveInvoiceEvent) => {
      setState((prev) => {
        const key = `total${event.type.charAt(0).toUpperCase()}${event.type.slice(1)}` as MetricKey;
        return {
          ...prev,
  ...
  const visibleEvents = useMemo(() => state.recentEvents.slice(0, 10), [state.recentEvents]);
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600 }}>Invoice Activity</h2>
        <span
          style={{
            display: 'inline-flex',
            alignItems: 'center',
