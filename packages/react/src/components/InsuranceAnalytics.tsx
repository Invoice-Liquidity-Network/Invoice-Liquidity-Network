...
export function InsuranceAnalytics({ className, style }: InsuranceAnalyticsProps): JSX.Element {
  ...
  const approvalRate = (approvedClaims?.length ?? 0) + (rejectedClaims?.length ?? 0) > 0
    ? ((approvedClaims?.length ?? 0) / ((approvedClaims?.length ?? 0) + (rejectedClaims?.length ?? 0))) * 100
    : 0;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', marginBottom: 20 }}>
        <StatsCard title="Pool Reserve" value={formatCurrency(reserve)} accentColor={POSITIVE} style={{ background: PANEL }} />
        <StatsCard title="Total Premiums" value={formatCurrency(totalPremiums)} accentColor={ACCENT} style={{ background: PANEL }} />
        <StatsCard title="Total Payouts" value={formatCurrency(totalPayouts)} accentColor={DANGER} style={{ background: PANEL }} />
        <StatsCard title="Payout Ratio" value={formatPercent(payoutRatio)} accentColor={WARNING} style={{ background: PANEL }} />
        <StatsCard title="Reserve Ratio" value={formatPercent(reserveRatio)} accentColor={reserveRatio > 0.3 ? POSITIVE : WARNING} style={{ background: PANEL }} />
        <StatsCard title="Approval Rate" value={`${approvalRate.toFixed(0)}%`} accentColor={approvalRate > 50 ? POSITIVE : WARNING} style={{ background: PANEL }} />
      </div>

      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
        <div style={{ padding: 18, borderRadius: 24, border: `1px solid ${BORDER}`, background: PANEL }}>
          <h3 style={{ margin: '0 0 12px 0', fontFamily: '"Newsreader", Georgia, serif', fontSize: 16 }}>Claims Breakdown</h3>
        ...
        <div style={{ padding: 18, borderRadius: 24, border: `1px solid ${BORDER}`, background: PANEL }}>
          <h3 style={{ margin: '0 0 12px 0', fontFamily: '"Newsreader", Georgia, serif', fontSize: 16 }}>Pool Health</h3>
          <div style={{ display: 'grid', gap: 10 }}>
            <div>
        ...
        <div style={{ padding: 18, borderRadius: 24, border: `1px solid ${BORDER}`, background: PANEL }}>
          <h3 style={{ margin: '0 0 12px 0', fontFamily: '"Newsreader", Georgia, serif', fontSize: 16 }}>Active Risk</h3>
          <div style={{ display: 'grid', gap: 10 }}>
            <MetricRow label="Active Claims" value={poolBalance?.activeClaims ?? 0} color={WARNING} />
            <MetricRow label="Pending Review" value={pendingClaims?.length ?? 0} color={WARNING} />
            <div style={{ fontSize: 11, color: MUTED, marginTop: 4 }}>
              Potential exposure: {formatCurrency(BigInt(poolBalance?.pendingClaims ?? 0) * (reserve > 0n ? reserve / BigInt(Math.max(poolBalance?.activeClaims ?? 1, 1)) : 0n))}
            </div>
          </div>
        </div>
      </div>
