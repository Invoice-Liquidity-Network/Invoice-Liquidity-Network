    ...
    return Promise.resolve({
      ...
      json: () =>
        Promise.resolve({
          ...
          _links: {
            self: {
              href: 'https://horizon.stellar.org/contracts/test/events?cursor=&limit=200&order=asc',
            },
...
describe('ILNEventIndexer', () => {
  ...
  describe('getEventsForInvoice', () => {
    it('throws TimeoutError when a Horizon query exceeds the configured timeout', async () => {
      ...
      const indexer = new ILNEventIndexer(CONTRACT_ID, { timeoutMs: 25 }, fetchMock);
    ...
    it('returns empty array when no events match', async () => {
      const events = [makeRawEvent({ topics: ['invoice-999'], tx_hash: 'tx1' })];
  ...
  describe('pagination', () => {
    it('fetches across multiple pages', async () => {
      ...
      const page2 = [makeRawEvent({ tx_hash: 'tx3', paging_token: '3', topics: ['invoice-001'] })];
  ...
  describe('getEventsForAddress', () => {
    ...
    it('filters by event type when types array is provided', async () => {
      ...
      expect(result).toHaveLength(2);
      expect(result.map((e) => e.type)).toEqual(['InvoiceCreated', 'InvoiceRepaid']);
    });
  });
