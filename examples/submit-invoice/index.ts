...
async function main() {
  ...
  console.log(
    `- Network: ${config.networkPassphrase === Networks.TESTNET ? 'Testnet' : 'Custom'}\n`
  );
  ...
  try {
    ...
    console.log(
      `🔗 Check on Stellar Explorer: https://stellar.expert/explorer/testnet/contract/${config.contractId}`
    );
